...
class EnsembleFlashAttention2(EnsembleAttention):
    def forward(
        ...
        # ==========================================================================================
        # query -> (TP * batch_size, query_length, num_heads, head_dim)
        # key -> (TP * batch_size, key_length, num_heads, head_dim)
        # value -> (TP * batch_size, key_length, num_heads, head_dim)
        # ==========================================================================================

        softmax_scale = self._get_softmax_scale()
        dropout_p = self.attn_pdrop if self.training else 0

        batch_size_tp, query_length = query.shape[:2]
        ...
        if attention_mask is None:
            ...
            key = index_first_axis(
                key.reshape(batch_size_tp * key_length, self.num_key_value_heads, self.head_dim), indices_k
            )
            value = index_first_axis(
                value.reshape(batch_size_tp * key_length, self.num_key_value_heads, self.head_dim), indices_k
            )

            if query_length == key_length:
                query = index_first_axis(
                    query.reshape(batch_size_tp * key_length, self.num_heads, self.head_dim), indices_k
                )
                ...
                cu_seqlens_q = torch.arange(
                    batch_size_tp + 1, dtype=torch.int32, device=query.device
            ...
            attn_output = pad_input(attn_output, indices_q, batch_size_tp, query_length)

        attn_output = attn_output.view(self.tp_world_size, -1, query_length, self.num_heads * self.head_dim)
