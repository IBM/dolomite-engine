...
from .linear import LadderColumnParallelLinear, LadderRowParallelLinear


class LadderMLP_TP(MLP):
    def __init__(
        ...
        self.c_fc = LadderColumnParallelLinear(
            hidden_size,
        ...
        if init_method == InitMethod.mup:
            std /= math.sqrt(m_width)
        self.c_proj = LadderRowParallelLinear(
            intermediate_size,
            hidden_size,
            bias=self.add_bias,
            std=std,
