...
class _RMSNorm_Cute_F(torch.autograd.Function):
    ...
    @ensure_contiguous
    def forward(
        ctx, x: torch.Tensor, weight: torch.Tensor | None, eps: float | None, sequence_parallel: bool
    ) -> torch.Tensor:
        ...
        _MLP_F.append((x, weight, rmsnorm_denominator, is_x_1d, eps, sequence_parallel, device_mesh))
...
def rmsnorm_cute_forward(
    ...
    x = wait_for_ACT(x, wait_in_forward=True, wait_in_backward=False)
    x = _RMSNorm_Cute_F.apply(x, weight, eps, sequence_parallel)
    return x
