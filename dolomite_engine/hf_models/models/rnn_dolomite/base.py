...
class RNNDolomitePreTrainedModel(PreTrainedModel):
    """
...
class RNNDolomiteModel(RNNDolomitePreTrainedModel):
    mask_value = None
    ...
    def _prepare_a_bunch_of_stuff(
        ...
        if self._use_padding_free_transformer:
            assert position_ids is not None, (
                "RNNDolomiteModel needs position_ids from outside when using flash attention with List[List[int]] "
                "inputs"
            )
        else:
            if self.position_embedding_type == PositionEmbeddingType.alibi:
