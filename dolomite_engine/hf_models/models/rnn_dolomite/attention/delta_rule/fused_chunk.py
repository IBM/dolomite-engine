...
from .wy_fast import bwd_prepare_wy_repr, fwd_prepare_wy_repr, fwd_recompute_w
...
if is_triton_available():
    ...
    @triton.autotune(
        configs=[triton.Config({}, num_warps=1), triton.Config({}, num_warps=2), triton.Config({}, num_warps=4)],
        key=["BT", "BK"],
    )
    @triton.jit
    def fused_chunk_delta_rule_fwd_kernel(
        ...
        p_o = tl.make_block_ptr(
            o + (i_bh + i_k * B * H) * s_vo_h, (T, DV), (s_vo_t, s_vo_d), (0, i_v * BV), (BT, BV), (1, 0)
        )
        p_v_new = tl.make_block_ptr(v_new + i_bh * s_vo_h, (T, DV), (s_vo_t, s_vo_d), (0, i_v * BV), (BT, BV), (1, 0))

        if USE_INITIAL_STATE:
            p_h = tl.make_block_ptr(
                initial_state + i_bh * DK * DV, (DK, DV), (DV, 1), (i_k * BK, i_v * BV), (BK, BV), (1, 0)
            )
    ...
    def fused_chunk_delta_rule_bwd_kernel(
        ...
        for i in range(tl.cdiv(T, BT) - 1, -1, -1):
            ...
            p_dk = tl.make_block_ptr(
                dk + (i_bh + i_v * B * H) * s_qk_h, (T, DK), (s_qk_t, s_qk_d), (i * BT, i_k * BK), (BT, BK), (1, 0)
            )
            p_dv = tl.make_block_ptr(
                dv + (i_bh + i_k * B * H) * s_vo_h, (T, DV), (s_vo_t, s_vo_d), (i * BT, i_v * BV), (BT, BV), (1, 0)
            )
            # [DK, BT]
            b_q = tl.load(p_q, boundary_check=(0, 1))
        ...
        if USE_INITIAL_STATE:
            p_h = tl.make_block_ptr(
                initial_state + i_bh * DK * DV, (DV, DK), (1, DV), (i_v * BV, i_k * BK), (BV, BK), (0, 1)
            )
            b_h += tl.load(p_h, boundary_check=(0, 1)).to(tl.float32)
        NT = tl.cdiv(T, BT)
        for i in range(0, NT):
            p_dv = tl.make_block_ptr(
                dv + i_bh * s_vo_h, (T, DV), (s_vo_t, s_vo_d), (i * BT, i_v * BV), (BT, BV), (1, 0)
            )
            b_dv = tl.load(p_dv, boundary_check=(0, 1))
            b_dd = tl.dot(b_dv.to(k.dtype.element_ty), b_h.to(k.dtype.element_ty), allow_tf32=False)
            p_dd = tl.make_block_ptr(
                dd + (i_bh + i_v * B * H) * s_qk_h, (T, DK), (s_qk_t, s_qk_d), (i * BT, i_k * BK), (BT, BK), (1, 0)
            )
            ...
            p_dq = tl.make_block_ptr(
                dq + (i_bh + i_v * B * H) * s_qk_h, (T, DK), (s_qk_t, s_qk_d), (i * BT, i_k * BK), (BT, BK), (1, 0)
            )
            # [BT, DK]
            b_k = tl.load(p_k, boundary_check=(0, 1))
            # [DV, BT]
...
class FusedChunkDeltaRuleFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, beta, BT, scale, initial_state, output_final_state):
        ...
        o, v_new, CHECK, final_state = fused_chunk_delta_rule_fwd(
            q, k, u, w, BT, scale, initial_state, output_final_state
        )
