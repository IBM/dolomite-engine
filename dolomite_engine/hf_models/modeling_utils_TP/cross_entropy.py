...
class _TensorParallelCrossEntropy(torch.autograd.Function):
    @record_function("TP:cross_entropy_forward")
    @staticmethod
    def forward(
    ...
    @record_function("TP:cross_entropy_backward")
    @staticmethod
...
def tensor_parallel_cross_entropy(
    lm_logits: torch.Tensor, labels: torch.Tensor, vocab_size: int, upcast_logits_for_loss: bool
) -> torch.Tensor:
    return _TensorParallelCrossEntropy.apply(lm_logits, labels, vocab_size, upcast_logits_for_loss)
