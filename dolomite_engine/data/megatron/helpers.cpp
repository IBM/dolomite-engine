                            ...
                            const bool verbose)
{
    ...
    if (verbose) { std::cout << "> building indices for blended datasets ..." << std::endl; }
                        ...
                        const int32_t min_num_sent)
{
    if (sizes_.size() > std::numeric_limits<uint32_t>::max()) {
        ...
        return build_mapping_impl<uint64_t>(
            docs_, sizes_, num_epochs, max_num_samples, max_seq_length, short_seq_prob, seed, verbose, min_num_sent);
    } else {
