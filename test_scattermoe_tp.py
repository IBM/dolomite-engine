...
c_proj_weight_ = c_proj_weight.view(config.num_experts, config.n_embd, config.n_inner)
load_dparams(
    shard_moe.c_proj,
    "weight",
...
torch.distributed.barrier()
print("Sharded.")
shard_out, shard_logits, _ = shard_moe(shard_input_tensor)
...
for r in range(tp_size):
    if rank == r:
        diff = (local_input_tensor_grad - shard_input_tensor_grad).abs()
