...
def load_checkpoint_for_training(model: DeepSpeedEngine, load_path: str) -> None:
...
def save_deepspeed_checkpoint(model: DeepSpeedEngine, args: TrainingArgs, save_path: str) -> None:
    ...
    save_training_args(args, os.path.join(save_path, f"global_step{model.global_steps}", "training_config.json"))


@register_timer("load_checkpoint_for_inference")
def load_checkpoint_for_inference(model: Model, load_path: str) -> None:
...
def convert_deepspeed_to_huggingface_checkpoint(model: Model, load_path: str, save_path: str) -> None:
    ...
    load_checkpoint_for_inference(model, load_path)
...
@run_rank_n
def save_training_args(args: TrainingArgs, save_path: str) -> None:
    ...
    # optimizer
    args.optimizer[OptimizerKeys.optimizer_class.value] = args.optimizer[OptimizerKeys.optimizer_class.value].__name__

    json.dump(vars(args), open(save_path, "w"), indent=4)


@run_rank_n
def save_inference_args(args: InferenceArgs, save_path: str) -> None:
